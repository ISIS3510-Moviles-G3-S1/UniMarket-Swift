...
class ClarifaiService: NSObject, ImageAnalysisService {
    ...
    func analyzeImage(_ image: UIImage) async throws -> AnalysisResult {
        ...
        guard APIConfig.isConfigured() else {
            throw ClarifaiServiceError.notConfigured(APIConfig.configurationErrorMessage())
        }
    ...
    private func encodeImageToBase64(_ image: UIImage) throws -> String {
        guard let jpegData = image.jpegData(compressionQuality: APIConfig.imageCompressionQuality) else {
            throw ClarifaiServiceError.imageEncodingFailed
        }
        
        // Check file size
        if jpegData.count > APIConfig.maxImageSize {
    ...
    private func buildRequest(with base64Image: String) throws -> URLRequest {
        guard let url = URL(string: "\(APIConfig.clarifaiBaseURL)/models/\(APIConfig.apparelModelID)/outputs") else {
    ...
    private func validateResponse(_ response: URLResponse) throws {
        ...
        case 200...299:
            break // Success
        case 400:
            throw ClarifaiServiceError.badRequest
        case 401, 403:
...
enum ClarifaiServiceError: LocalizedError {
    ...
    var recoverySuggestion: String? {
        switch self {
        case .notConfigured:
            return "Set up your Clarifai API key in APIConfig.swift"
        case .imageTooLarge:
